...
(define (make-avl-tree equal? less?)
  ...
  (define (dot-plot file)
    (let ((seq '()) (port (open-output-file file)))
      (for-each
        (lambda (x)
          (if (not (null? (node-left x)))
            (set! seq (append! seq (list (cons (node-item x) (node-item (node-left x)))))))
          (if (not (null? (node-right x)))
            (set! seq (append! seq (list (cons (node-item x) (node-item (node-right x)))))))))
      (format port "graph avltree {~%")
      (let iter ((pair seq))
        (if (null? pair)
          '()
          (begin
            (format port "~A -- ~A;~%" (caar pair) (cdar pair))
            (iter (cdr pair)))))
      (format port "}")
